...
namespace WindowsFormsApp1
{
    public partial class FrmTicTacToe : Form
    {
            ...
            else
            {
                if (engine.GetCurrentTurn() == GameEngine.WhooseTurn.Player2CPU)
                {
                    ...
                    if (!cellChosenByCpu.IsErrorCell())
                    {
                        ...
                        {
                            MessageBox.Show("Произошла ошибка: выбранная компьютером клетка не должна быть равна null!", "Крестики-Нолики", 
                                MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                    else
                    {
                        MessageBox.Show( "Произошла ошибка: компьютер не смог выбрать клетку для хода!", "Крестики-Нолики",
                            MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
        ...
        private void RegularButtonMouseOver(Panel panelButton, Label labelButtonText)
        {
            ...
            labelButtonText.BackColor = Color.Purple;
            Cursor = Cursors.Hand;
        }
        private void RegularButtonMouseOut(Panel panelButton, Label labelButtonText)
        {
            ...
            labelButtonText.BackColor = Color.SlateBlue;
            Cursor = Cursors.Default;
        }
        ...
        private void labelNewGame_Enter(object sender, EventArgs e)
        {
            RegularButtonMouseOver(panelNewGame, labelNewGame);
        }
        ...
        private void UpdateControls()
        {
            ...
            labelWhooseTurn.Text = "Ход игрока N";
